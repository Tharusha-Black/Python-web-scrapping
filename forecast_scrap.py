...
class Scraper:
    ...
    def scrape_forecast_from_url(url):
        ...
        if response.status_code == 200:
            ...
            forecast_collection = ForecastCollection()  # Initialize forecast collection
            
            day_elements = soup.find_all('div', class_='day')  # Find all day elements
            ...
            for day_element in day_elements:
                ...
                # Extracting forecast data for the day
                ...
                condition_element = day_element.find('ul').find('li', string=re.compile(r'(Bright|Light|Morning|Afternoon|Cloudy|Clear|Mostly|Chance of a thunderstorm|Possible shower|Slight chance of rain|Cloudy with rain expected|Chance of Showers|Possible Showers).*', re.IGNORECASE))
                ...
                # Creating a Forecast object for each day's data
            ...
            return forecast_collection  # Return the collection of forecasts for all days
        else:
...
class CSVWriter:
    ...
    def write_forecast_data(filename, forecast_collection):
        with open(filename, 'a', newline='') as file:  # Open the file in append mode
            ...
            writer.writerow(["Location", "Updated_Date", "Forecast_Date", "Min_Temp", "Max_Temp", "Condition", "Possible_Rainfall", "Chance_of_Rain", "Forecast", "Warning"])
